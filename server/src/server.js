...
app.post('/espSource_set', async (req, res) => {
  const { sourceName, newStateOnlyForPower } = req.body

  const audioSource_Function = sourceName === 'Power'
    ? 'audioSource_togglePower'
    : 'audioSource_changeSource'

  const { success, error, errorMessage } = await AudioManager[audioSource_Function]({ IrManager, targetSourceName: sourceName, newState: newStateOnlyForPower, VideoControlManager })
...
app.post('/lights_animation', async (req, res) => {
  ...
  const lightsConfig = {
    animationId: !animationNewState ? 'v' : getAnimationFromId(animationName)
  }
