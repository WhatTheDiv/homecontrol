...
app.post('/espSource_set', async (req, res) => {
  const { commandName, newStateOnlyForPower } = req.body
  const audioSource_Function = commandName === 'Power'
    ? 'audioSource_togglePower'
    : 'audioSource_changeSource'

  const { success, error, errorMessage } = await AudioManager[audioSource_Function]({ IrManager, commandName })
